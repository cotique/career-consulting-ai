...
- **The corpus does not live here, and that is the first thing to settle.** This repository holds eleven markdown files, all of them project artifacts — skills, these two documents, the prompt templates. The material this feature is meant to answer over sits in a personal notes folder outside the repository, so ingestion reads from a path supplied at run time rather than from anything under version control. Two consequences follow and neither is plumbing. First, ingesting it copies personal career material into the application database, which puts it under the same erasure and export obligations as everything else the user owns (NFR6) — the corpus is not a read-only side input. Second, that folder keeps **dated backup snapshots of its own contents**: ingesting them wholesale would fill the index with near-identical chunks of superseded versions of the same document, so retrieval would return the stale copy as readily as the current one. Selecting what is ingested is therefore part of the design, not a later tuning step.
